...
btnForm.onclick = function() {
    ...
    if(form.name.value.length < 4) {
        ...
        document.querySelector("#error").innerHTML = "Длина комментариев не менее 10 символов";
        return false;
    }
    document.querySelector("#error").innerHTML = "";

    // установим новое значение подсчета комментариев
    if(countComments == 0)
        document.querySelector("#comments").innerHTML = "";
    countComments++;
    document.querySelector(".count-com").innerHTML = countComments;

    // создание самого комментария
    ...
    document.querySelector("#comments").insertAdjacentHTML('afterbegin', newComment); // beforend элементы вставляются в конце блока
